...
def chat():
    ...
    with sqlite3.connect(DB_NAME) as conn:
        row = conn.execute('SELECT avatar FROM users WHERE username = ?', (session['username'],)).fetchone()
...
# API: заявки в друзья, списки заблокированных
...
def delete_group():
    ...
    with sqlite3.connect(DB_NAME) as conn:
        ...
        c.execute('DELETE FROM groups WHERE room_id = ?', (room_id,))
        c.execute('DELETE FROM user_chats WHERE room_id = ?', (room_id,))
        c.execute('DELETE FROM messages WHERE room = ?', (room_id,))
        conn.commit()
    socketio.emit('group_deleted', {'room_id': room_id}, to=room_id)
    return jsonify({'success': True})
...
def upload_file():
    ...
    with sqlite3.connect(DB_NAME) as conn:
        ...
        row = c.execute('SELECT avatar FROM users WHERE username = ?', (session['username'],)).fetchone()
        avatar = row[0] if row else ''
        conn.commit()
...
def proxy_media():
    """Прокси для видео/медиа с внешних URL (обход CORS, чтобы видео воспроизводилось)."""
    ...
    try:
        ...
        r.raise_for_status()
        return Response(r.iter_content(chunk_size=8192), content_type=r.headers.get('Content-Type', 'video/mp4'))
    except Exception:
        return '', 502
